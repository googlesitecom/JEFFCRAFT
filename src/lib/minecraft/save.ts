...
export interface SavedWorld {
  ...
  // Armor state [helmet, chestplate, leggings, boots] (-1 = empty)
  armor: number[];
  // Timestamp
  savedAt: number;
}
...
export function saveWorld(world: World, name: string, seed: number, mode: "creative" | "survival", player: { position: { x: number; y: number; z: number }; yaw: number; pitch: number; health: number; hunger: number }, inventory: Inventory, dayTime: number, xp: { level: number; progress: number }, armor: number[]): boolean {
  try {
