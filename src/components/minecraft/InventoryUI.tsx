...
export function InventoryUI({
  ...
  // Render a slot — Minecraft-style inset bevel
  const renderSlot = (
    ...
    return (
      <div
        ...
        onTouchEnd={(e) => { e.preventDefault(); if (!disabled) onClick(false); }}
        className={`w-12 h-12 flex items-center justify-center relative transition-all ${
          disabled ? "opacity-40" : "cursor-pointer hover:brightness-110"
        } ${isControllerCursor ? "scale-110 z-20" : ""}`}
        style={{
          imageRendering: "pixelated",
          backgroundColor: disabled ? "#555" : "#8b8b8b",
          // Minecraft slot bevel: dark top-left, light bottom-right (inset look)
          borderTop: isControllerCursor ? "2px solid #4a8aff" : "2px solid #373737",
          borderLeft: isControllerCursor ? "2px solid #4a8aff" : "2px solid #373737",
          borderBottom: isControllerCursor ? "2px solid #4a8aff" : "2px solid #ffffff",
          borderRight: isControllerCursor ? "2px solid #4a8aff" : "2px solid #ffffff",
          boxShadow: isControllerCursor
            ? "0 0 12px rgba(100,150,255,0.9)"
  ...
  return (
    ...
    >
      <div
        className="relative p-4 max-h-[95vh] overflow-y-auto"
        style={{
          ...
          boxShadow: "inset 0 0 0 2px #555, 0 12px 40px rgba(0,0,0,0.8)",
        }}
        onClick={(e) => e.stopPropagation()}
      >
        {/* Close button — top-right corner, Minecraft style */}
        <button
          onClick={onClose}
          onTouchEnd={(e) => { e.preventDefault(); onClose(); }}
          className="absolute top-1 right-1 w-6 h-6 flex items-center justify-center text-white text-xs font-bold z-30"
          style={{
        ...
        {/* === TOP SECTION: Armor + Crafting + Recipe Book === */}
        <div className="flex items-start gap-3 mb-4">
          {/* Character model + armor slots (survival only, not crafting table) */}
          {!isCraftingTable && (
            <div className="flex flex-col items-center gap-1">
              {/* Armor slots - vertical column */}
                ...
                {(["helmet", "chestplate", "leggings", "boots"] as const).map((slot, armorIdx) => {
                  ...
                  return (
                    <div
                      ...
                      onTouchEnd={(e) => { e.preventDefault(); if (canEquip) handleArmorSlotClick(slot, false); }}
                      className={`w-12 h-12 flex items-center justify-center relative ${canEquip ? "cursor-pointer" : ""} ${isArmorFocused ? "scale-110 z-20" : ""}`}
                      style={{
                        imageRendering: "pixelated",
                        backgroundColor: "#8b8b8b",
                        borderTop: isArmorFocused ? "2px solid #4a8aff" : "2px solid #373737",
                        borderLeft: isArmorFocused ? "2px solid #4a8aff" : "2px solid #373737",
                        borderBottom: isArmorFocused ? "2px solid #4a8aff" : "2px solid #ffffff",
                        borderRight: isArmorFocused ? "2px solid #4a8aff" : "2px solid #ffffff",
                        boxShadow: isArmorFocused ? "0 0 12px rgba(100,150,255,0.9)" : "inset 1px 1px 0 rgba(0,0,0,0.15)",
                      }}
                    ...
                    >
                      {equippedId !== null && (() => {
                        ...
                        return (
                          <>
                            <img src={icon} alt="armor" className="w-9 h-9 relative z-10" style={{ imageRendering: "pixelated", filter: "drop-shadow(1px 1px 0 rgba(0,0,0,0.55))" }} draggable={false} />
                            <div className="absolute bottom-0.5 left-1 right-1 h-[3px] z-15" style={{ backgroundColor: "rgba(0,0,0,0.7)" }}>
                              <div className="h-full" style={{ width: `${durFraction * 100}%`, backgroundColor: durFraction > 0.5 ? "#6ade40" : durFraction > 0.2 ? "#facc15" : "#ef4444" }} />
                            </div>
                          </>
                        );
                      })()}
              ...
              {/* Character model (Steve) */}
              <div className="w-16 h-24 mt-1 relative" style={{ imageRendering: "pixelated" }}>
                <div className="absolute top-0 left-2 w-6 h-6 bg-[#b8845c]" style={{ boxShadow: "inset 2px 2px 0 #d4a378, 0 0 0 1px #5a3a20" }} />
                <div className="absolute top-0 left-2 w-6 h-1.5 bg-[#4a3020]" />
                <div className="absolute top-6 left-1.5 w-7 h-6 bg-[#4a8a3a]" style={{ boxShadow: "inset 2px 2px 0 #6aaa5a, 0 0 0 1px #2a4a1a" }} />
                <div className="absolute top-6 left-0 w-1.5 h-6 bg-[#b8845c]" style={{ boxShadow: "0 0 0 1px #5a3a20" }} />
                <div className="absolute top-6 right-0 w-1.5 h-6 bg-[#b8845c]" style={{ boxShadow: "0 0 0 1px #5a3a20" }} />
                <div className="absolute top-12 left-2 w-2.5 h-5 bg-[#3a3a6a]" style={{ boxShadow: "0 0 0 1px #1a1a3a" }} />
                <div className="absolute top-12 left-5 w-2.5 h-5 bg-[#3a3a6a]" style={{ boxShadow: "0 0 0 1px #1a1a3a" }} />
              </div>
            </div>
          )}
          ...
          <div className="flex flex-col items-center gap-1">
            {/* Crafting grid */}
            <div className="grid grid-cols-3 gap-0.5 p-1" style={{
          ...
          <div className="flex items-center gap-2">
            <div className="text-[#555] text-2xl font-bold">→</div>
            {/* Result slot — highlighted with yellow border */}
            <div className="p-1" style={{
              ...
              {result ? (
                <div
                  ...
                  onTouchEnd={(e) => { e.preventDefault(); handleTakeResult(false); }}
                  className={`w-12 h-12 flex items-center justify-center cursor-pointer ${controllerActive && controllerZone === "result" ? "scale-110" : "hover:scale-105"}`}
                  style={{
                    imageRendering: "pixelated",
                    backgroundColor: "#8b8b8b",
                    ...
                    boxShadow: controllerActive && controllerZone === "result"
                      ? "0 0 12px rgba(100,150,255,0.9)"
                      : "0 0 8px rgba(255,205,48,0.4)",
                  }}
                >
                  {(() => {
                    const icon = getIcon(result.id);
                    return icon ? (
                      <img src={icon} alt={getName(result.id)} className="w-9 h-9 relative z-10" style={{ imageRendering: "pixelated", filter: "drop-shadow(1px 1px 0 rgba(0,0,0,0.55))" }} draggable={false} />
                    ) : null;
                  })()}
                  {result.count > 1 && (
                    <span className="absolute bottom-0 right-1 text-white text-xs font-mono font-bold z-20" style={{ textShadow: "2px 2px 0 #000, -1px -1px 0 #000, 1px -1px 0 #000, -1px 1px 0 #000" }}>
                      {result.count}
                    </span>
                  )}
                </div>
          ...
          {/* Recipe book toggle — green book icon, Minecraft style */}
          <button
            onClick={() => setShowRecipeBook(!showRecipeBook)}
            onTouchEnd={(e) => { e.preventDefault(); setShowRecipeBook(!showRecipeBook); }}
            className="w-12 h-12 flex items-center justify-center text-2xl z-10"
            style={{
              backgroundColor: showRecipeBook ? "#3a6a2a" : "#4a8a3a",
              borderTop: "2px solid #6aaa5a",
              borderLeft: "2px solid #6aaa5a",
              borderBottom: "2px solid #2a4a1a",
              borderRight: "2px solid #2a4a1a",
              imageRendering: "pixelated",
              cursor: "pointer",
              boxShadow: showRecipeBook ? "inset 2px 2px 4px rgba(0,0,0,0.4)" : "none",
            }}
            title="Recipe Book"
          >
            <span style={{ filter: "drop-shadow(1px 1px 0 rgba(0,0,0,0.5))" }}>📖</span>
          </button>
        </div>

        {/* === RECIPE BOOK (collapsible) === */}
        {showRecipeBook && (
          <div className="mb-4 p-2" style={{
            ...
            <div className="grid grid-cols-6 sm:grid-cols-8 gap-0.5 max-h-48 overflow-y-auto">
              {allRecipes.map((recipe, i) => {
                ...
                return (
                  <button
                    ...
                    onTouchEnd={(e) => { e.preventDefault(); if (canCraft && !needsTable) handleRecipeClick(recipe); }}
                    className={`w-12 h-12 flex items-center justify-center relative ${isRecipeFocused ? "scale-110 z-20" : ""}`}
                    style={{
                      imageRendering: "pixelated",
                      backgroundColor: isRecipeFocused ? "#b8d0ff" : "#8b8b8b",
                      borderTop: isRecipeFocused ? "2px solid #4a8aff" : "2px solid #373737",
                      borderLeft: isRecipeFocused ? "2px solid #4a8aff" : "2px solid #373737",
                      borderBottom: isRecipeFocused ? "2px solid #4a8aff" : "2px solid #ffffff",
                      borderRight: isRecipeFocused ? "2px solid #4a8aff" : "2px solid #ffffff",
                      boxShadow: isRecipeFocused ? "0 0 12px rgba(100,150,255,0.9)" : "inset 1px 1px 0 rgba(0,0,0,0.15)",
                      cursor: (canCraft && !needsTable) ? "pointer" : "help",
                      opacity: (!canCraft && !needsTable) ? 0.5 : 1,
                    }}
                    title={getName(recipe.result.id) + (needsTable ? " (requires table)" : canCraft ? "" : " (no materials)")}
                  >
                    {recipeIcon ? (
                      <img src={recipeIcon} alt={getName(recipe.result.id)} className="w-9 h-9 relative z-10" style={{ imageRendering: "pixelated", filter: "drop-shadow(1px 1px 0 rgba(0,0,0,0.55))" }} draggable={false} />
                    ) : (
                    ...
                    {needsTable && (
                      <span className="absolute bottom-0 right-0 text-[9px] text-white font-mono font-bold px-0.5 z-20" style={{ backgroundColor: "#3a5a8a", textShadow: "1px 1px 0 #1a2a4a" }}>T</span>
                    )}
                  </button>
                );
              })}
        ...
        {/* === CREATIVE ITEMS (creative mode only) === */}
        {isCreative && (
          <div className="mb-4 p-2" style={{
            backgroundColor: "#373737",
            borderTop: "2px solid #1a1a1a",
            ...
            <div className="grid grid-cols-9 sm:grid-cols-12 gap-0.5 max-h-32 overflow-y-auto">
              {creativeItems.map((id, ci) => {
                ...
                return (
                  <button
                    ...
                    onTouchEnd={(e) => { e.preventDefault(); handleCreativeItemClick(id); }}
                    className={`w-12 h-12 flex items-center justify-center cursor-pointer ${isCreativeFocused ? "scale-110 z-20" : ""}`}
                    style={{
                      imageRendering: "pixelated",
                      backgroundColor: isCreativeFocused ? "#b8d0ff" : "#8b8b8b",
                      borderTop: isCreativeFocused ? "2px solid #4a8aff" : "2px solid #373737",
                      borderLeft: isCreativeFocused ? "2px solid #4a8aff" : "2px solid #373737",
                      borderBottom: isCreativeFocused ? "2px solid #4a8aff" : "2px solid #ffffff",
                      borderRight: isCreativeFocused ? "2px solid #4a8aff" : "2px solid #ffffff",
                      boxShadow: isCreativeFocused ? "0 0 12px rgba(100,150,255,0.9)" : "inset 1px 1px 0 rgba(0,0,0,0.15)",
                    }}
                    title={name}
                  >
                    <img src={icon} alt={name} className="w-9 h-9 relative z-10" style={{ imageRendering: "pixelated", filter: "drop-shadow(1px 1px 0 rgba(0,0,0,0.55))" }} draggable={false} />
                  </button>
                );
              })}
            </div>
        ...
        {/* === MAIN INVENTORY (9×3 grid) — Minecraft style, no label === */}
        <div className="grid grid-cols-9 gap-0.5 p-1 mb-2" style={{
          backgroundColor: "#373737",
          borderTop: "2px solid #1a1a1a",
          borderLeft: "2px solid #1a1a1a",
          borderBottom: "2px solid #c6c6c6",
        ...
        {/* === HOTBAR (9×1) — separated by a small gap like Minecraft === */}
        <div className="grid grid-cols-9 gap-0.5 p-1" style={{
          backgroundColor: "#373737",
          borderTop: "2px solid #1a1a1a",
          borderLeft: "2px solid #1a1a1a",
          borderBottom: "2px solid #c6c6c6",
        ...
        {/* Hint text — minimal, Minecraft doesn't show hints but we keep for usability */}
        {controllerActive && (
          <p className="text-[#3a5a8a] text-xs font-mono text-center mt-2" style={{ textShadow: "1px 1px 0 #ddd" }}>
            🎮 D-Pad: move · A: left click · X: right click · LB/RB: zone · B: close
          </p>
        )}
      </div>
    </div>
